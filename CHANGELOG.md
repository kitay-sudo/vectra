...
- Автозагружаемая точка входа: установщик кладёт в корень `AGENTS.md` и `CLAUDE.md` (блок `install/bootstrap.md`), поэтому в новой сессии агент следует Vectra без вставки команд. Существующий агентский файл не перезаписывается — блок дописывается между маркерами `vectra:start`/`vectra:end`.
